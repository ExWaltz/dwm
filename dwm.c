...
<<<<<<< HEAD
	    // drw_rect(drw, x + boxw - 1.2, (boxw * 3)+3, w - (1.2 * boxw + 1), boxw - 20,
        //         m == selmon && selmon->sel && selmon->sel->tags & 1 << i ? 0 : 1, 
        //         urg & 1 << i);
=======
		if (occ & 1 << i)
			drw_rect(drw, x + boxw - 1.2, (boxw * 3)+3, w - (1.2 * boxw + 1), boxw - 2,
...
void
runAutostart(void) {
	system("cd ~/.config/X11/; ./autostart &");
}
